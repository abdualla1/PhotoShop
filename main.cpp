...
class ImageFilter {
public:
    static void invertFilter(Image &img) {
    ...
    static void rotate90(Image &img, Image &newImg) {
    for (int i = 0; i < img.width; i++) {
        for (int j = 0; j < img.height; j++) {
            for (int c = 0; c < img.channels; c++) {
...
static void rotate180(Image &img, Image &newImg) {
    for (int i = 0; i < img.width; i++) {
        for (int j = 0; j < img.height; j++) {
            for (int c = 0; c < img.channels; c++) {
...
static void rotate270(Image &img, Image &newImg) {
    for (int i = 0; i < img.width; i++) {
        for (int j = 0; j < img.height; j++) {
            for (int c = 0; c < img.channels; c++) {
    ...
    static void blurFilter(Image &img, int blurLevel) {
...
static void addFrame(Image &img, string style, string color) {
...
int main() {
    ...
    do {
        ...
        switch (choice) {
            case 1:
                ImageFilter::invertFilter(img);
                img.saveImage(outputFilename);
                cout << "Invert filter applied successfully.\n";
                break;
            case 2:
                ImageFilter::rotate90(img, img90);
                cout << "Image rotated 90 degrees successfully.\n";
                img90.saveImage(outputFilename);
                break;
            case 3:
                ImageFilter::rotate180(img, img180);
                cout << "Image rotated 180 degrees successfully.\n";
                img180.saveImage(outputFilename);
                break;
            case 4:
                ImageFilter::rotate270(img, img90);
            ...
            case 5:
                ...
                cin >> blurLevel;
                ImageFilter::blurFilter(img, blurLevel);
                img.saveImage(outputFilename);
            ...
            case 6:
                ...
                cin >> color;
                ImageFilter::addFrame(img, style, color);
                img.saveImage(outputFilename);
